...
class HandleParamClass
{
    const T_RAW			 = 0x0;  //0
    const T_STRING		 = 0x1; //1
    const T_INT			 = 0x2;//10
    const T_FLOAT		 = 0x4;//100
    const T_BOOL		 = 0x8;//1000
    const T_MAP			 = 0x10;//10000
    const T_STRIP_ER	 = 0x20; //strip \r 100000
    const T_STRIP_NL	 = 0x40; //strip \n and \r 1000000
    const T_STRIP_TAGS	 = 0x80; //strip html tags 10000000
    const T_STRING_STRIC = 0x100; //敏感词检查 100000000
    const T_JSON		 = 0x200; //1000000000
    const T_EMAIL		 = 0x400;//10000000000
    const T_URL			 = 0x800;//100000000000
    const T_PHONE		 = 0x1000;//1000000000000
    const T_MOBILE		 = 0x2000;//10000000000000
    ...
    /**
     * check whether content contain sensitive words
     * @param unknown $data
     */
    public static function check( $content ){
        $words = SensitiveWordTable::instance()->get_sensitive_word();
        ...
        return preg_match($reg , $content)?  true : false;
    }

    // check senstive words -- off to interface
    public static function is_sensitive($word)
    {
        $pattern_array = Core::get_config('const', 'sensitive_word');
