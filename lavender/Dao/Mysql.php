...
class Mysql
{
	...
	{
		if ( defined('SQL_DEBUG') && SQL_DEBUG ) echo "\n<br><b style=\"color:red\">[{$this->database}]<br><i style=\"color:blue\">{$sql}</i></b><br>\n";
		$this->last_sql = $sql;
		$filename = 'sql';
//echo "\n".$this->last_sql."\n";
		$lower = strtolower($this->last_sql);
		if (preg_match('/order/', $lower) && preg_match('/pay_time/', $lower) && (preg_match('/update /', $lower) || preg_match('/insert /', $lower))) {
			$option = debug_backtrace();
			Core::log('debug_trace_db_order', date('Y-m-d H:i:s') . ' -- ' . var_export($option, true) . "\n");
		}

		// $content .= "ip: {$ip}\n";

		$content = date('Y-m-d H:i:s').' - ';
		$content .= \Golo\GoloWebService::$user_id . ' - ';
		$time_1 = microtime(true);

		$this->open();

		//是否在sql中含有serial_no
		$serial_pos = strpos($sql,'serial_no');

		if($serial_pos !== false){
			$sql = $this->_addquote_to_serial_no($sql);
		}
		$result = mysqli_query($this->db_link, $sql);
	...
	public function insert($table, array $record)
	{
		...
		foreach ($record as $k => $v) {
			...
			if (is_array($v)) {
				$is_two = true;
				$insert_fields = array_keys($v);
				$insert_values[] = '(\'' . implode('\',\'', array_values($v)) . '\')';
			} else {
				$insert_fields[] = $k;
				$insert_values[] = $v;
			}
	...
	public function escape_array($items)
	{
		...
		foreach ($items as $k => $v) {
			if (is_array($v)) {
				foreach ($v as $k2 => $v2) {
					$data[$k][$k2] = mysqli_real_escape_string($this->db_link, $v2);
				}
			} else {
				$data[$k] = mysqli_real_escape_string($this->db_link, $v);
			}
		}
	...
	{
		if (preg_match('/^(distinct)/', $string)) {
			list($empty, $string) = explode('distinct', $string);
			$string = trim($string);
		}
	...
	/**
	 * add quotes to sql serial_no=9699 => serial_no='9699...'
	 *
	 * @param string $sql SELECT
	 *
	 * @return string $sql
	 */
	private function _addquote_to_serial_no($sql){

			$num = preg_match_all('/\`serial_no\`\s*=\s*\d+/',$sql,$matches);

			if(!$num){//serial_no不带执行符号时候
				$num = preg_match_all('/serial_no\s*=\s*\d+/',$sql,$matches);
			}
			...
			if($num > 0){
				foreach($matches[0] AS $kk => $vv){
					//去掉空格字符
					$temp_arr = explode('=',$vv);
					$matches_str = trim($temp_arr[0]).'='.'\''.trim($temp_arr[1]).'\'';

					$sql = str_replace($vv,$matches_str,$sql);
